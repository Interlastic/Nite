...
function simulateResponse() {
    ...
    if(conversations[currentConversation]) conversations[currentConversation].messages.push({text:response,sent:false,time});
...
function sendNewMessage() {
    ...
    conversations[to] = {avatar,gradient:'linear-gradient(135deg,hsl(' + Math.random()*360 + ',70%,60%) 0%,hsl(' + Math.random()*360 + ',70%,50%) 100%)',messages:[{text,sent:true,time:new Date().toLocaleTimeString('en-US',{hour:'numeric',minute:'2-digit',hour12:true})}]};
