...
document.addEventListener('DOMContentLoaded',() => {loadNotes(); autoSave(); });
function loadNotes() {
...
function backToList() { saveCurrent(); document.getElementById('notes-list-screen').classList.add('active'); document.getElementById('note-editor-screen').classList.remove('active'); renderList(); cni=null; }
...
function autoSave() {
    ['note-title','note-content'].forEach(id => { const e=document.getElementById(id); if(e) e.addEventListener('input',saveCurrent); });
}
function deleteCurrent() {
    if(cni!==null && confirm('Delete?')) { notes=notes.filter(x=>x.id!==cni); saveNotes(); backToList(); }
}
